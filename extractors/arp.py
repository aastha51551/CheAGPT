...
def visit_body(text, cm, tm, font_dict, font_size):
    if (
        ...
        and font_dict["/BaseFont"] == "/DAAAAA+DejaVuSans"
    ):
...
# Debug: Print out bold texts
for i, text in enumerate(bold_texts, start=1):
    print(f"Bold text {i}: {text}")
...
output_path = output_dir / "arp.jsonl"
